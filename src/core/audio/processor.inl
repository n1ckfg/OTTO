...
namespace otto::core::audio {
  ...
  template<int N>
  template<std::size_t NN>
  ProcessData<NN> ProcessData<N>::redirect(const AudioBufferHandleArray<NN>& buf)
  {
    return ProcessData<NN>{buf, midi, nframes};
  }
  ...
  template<std::size_t NN>
  ProcessData<NN> ProcessData<0>::redirect(const AudioBufferHandleArray<NN>& buf)
  {
    return ProcessData<NN>{buf, midi, nframes};
  }
  ...
  template<std::size_t NN>
  inline ProcessData<NN> ProcessData<1>::redirect(const AudioBufferHandleArray<NN>& buf)
  {
    return ProcessData<NN>{buf, midi, nframes};
  }
